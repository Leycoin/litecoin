...
#if defined(NDEBUG)
# error "leycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x41aca811fa61f61879cc4880f26d6c4248f74cd113df5ec420e14b2783339536");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // leycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "leycoin Signed Message:\n";
...
{
    // leycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // leycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // leycoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // leycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // leycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // leycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "1SBCT Deploys 04/04/2018 to undisclosed location";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047bddd806962a51cd68af67f337e8772e8873aeda5f0a9e3a5326020b2a611f0573f8a9c829daecd9684077fe6cb878626b0cc60dbb54a1b9ad416c633a00fb22") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520915627;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085951059;
        ...
        assert(block.hashMerkleRoot == uint256("0x972f64317cf7ae104fa954046177138d4352ea3ac73cb85480a7000f23c3df48"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xb2, 0xd6 }; // leycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("leycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("leycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static leycoinMiner(CWallet *pwallet)
{
    printf("leycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("leycoin-miner");
    ...
    try { loop {
        ...
        printf("Running leycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("leycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&leycoinMiner, pwallet));
}
